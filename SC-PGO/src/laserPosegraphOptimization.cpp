...
void savedata(const sensor_msgs::PointCloud2 &pointcloud, const nav_msgs::Odometry &odomAftPGOthis, int node_idx)
{
    char id[8];
    int i = snprintf(id, 5, "%05d", node_idx);
    std::string folder_path(save_path + "/" + id);   
    std::cout << "folder_path= " << folder_path << std::endl;
    const char* p = folder_path.data();
	//if(access(p, 0) == -1)
	//{
		//mkdir(p, S_IRWXO);
	//}
    std::string pcd_path(folder_path + ".pcd");
    //std::cout << "pcd_path= " << pcd_path << std::endl;
    pcl::PointCloud<pcl::PointXYZ> cloud;
    pcl::fromROSMsg(pointcloud, cloud);
    pcl::io::savePCDFileASCII (pcd_path, cloud);

    std::string odom_path(folder_path + ".odom");
    //std::cout << "odom_path= " << odom_path << std::endl;

    Eigen::Quaterniond q;
    q.x() = odomAftPGOthis.pose.pose.orientation.x;
    q.y() = odomAftPGOthis.pose.pose.orientation.y;
    q.z() = odomAftPGOthis.pose.pose.orientation.z;
    q.w() = odomAftPGOthis.pose.pose.orientation.w;
    ...
    matrix4d(0,3) = odomAftPGOthis.pose.pose.position.x;
    matrix4d(1,3) = odomAftPGOthis.pose.pose.position.y;
    matrix4d(2,3) = odomAftPGOthis.pose.pose.position.z;


    std::ofstream laser_odom;
    laser_odom.open(odom_path, std::ios_base::app); //<< std::setw(20) 
    for (int i = 0; i < 3; ++i)
    {
        for (int j = 0; j < 4; ++j)
        {
            laser_odom << matrix4d(i, j);
            if (j == 3)
            {
                laser_odom << std::endl;
            }
...
void process_interactive_slam_result() {
  ...
  for (int node_idx=0; node_idx < int(keyframePosesUpdated.size()); node_idx++) {
    ...
    //std::cout << laserCloudMapPGOMsg << std::endl;
    ...
    savedata(laserCloudMapPGOMsg, odomAftPGOthis, node_idx);
    rate.sleep();
    //std::cout << odomAftPGOthis << std::endl;
  }
  mKF.unlock();
} // pointcloud_viz
